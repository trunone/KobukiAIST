...
void Translator::onDockingIR(const Packet& packet)
{
  for(int i = BUFFER_LENGTH-2;i >= 0 ;i--) {
    m_pKobuki->m_DockRightSignalBuffer[i+1] = m_pKobuki->m_DockRightSignalBuffer[i];
    m_pKobuki->m_DockCenterSignalBuffer[i+1] = m_pKobuki->m_DockCenterSignalBuffer[i];
    m_pKobuki->m_DockLeftSignalBuffer[i+1] = m_pKobuki->m_DockLeftSignalBuffer[i];
  }
  m_pKobuki->m_DockRightSignalBuffer[0]  = packet[2];
  m_pKobuki->m_DockCenterSignalBuffer[0] = packet[3];
  m_pKobuki->m_DockLeftSignalBuffer[0]   = packet[4];
...
{
    m_pKobuki->m_GyroFrameID = packet[2];
    m_pKobuki->m_GyroDataLength = packet[3];
    const double digit_to_dps = 0.00875;
    for(int offset = 4, i = 0; offset<2*m_pKobuki->m_GyroDataLength+4; offset+=6, i++) {
        m_pKobuki->m_Gyro[i].X =  -digit_to_dps * (short)packet.ushort(offset+2);
        m_pKobuki->m_Gyro[i].Y =   digit_to_dps * (short)packet.ushort(offset);
        m_pKobuki->m_Gyro[i].Z =   digit_to_dps * (short)packet.ushort(offset+4);
    }
}
